    ...
    const validateCancelRecon = async function (res) {
        return new Promise((resolve, reject) => {
            ...
            res.on('end', () => {
                // Add validation on 'responseBody' here if required.
                accessToken = JSON.parse(responseBody).access_token;
                log.info(`Cancel recon response : ${responseBody}`)
                resolve();
            });
        });
    };
    ...
    const validateReconDuration = async function (res) {
        ...
        return new Promise((resolve, reject) => {
            ...
            res.on('end', async () => {
                ...
                log.info(`Number of Reconciliations (in any state) found: ${JSON.parse(responseBody).reconciliations.length}`)
                JSON.parse(responseBody).reconciliations.forEach(recon => {
                    log.info(`Reconciliation found: _id=${recon._id} - mapping: ${recon.mapping} - state: ${recon.state}  - duration (secs): ${Math.floor(recon.duration / 1000)}`)
                })
                log.info(`Active Reconciliations found: ${activeReconsData.length}`)
                await Promise.all(activeReconsData.map(async (activeRecon) => {
                    ...
                    if (activeRecon && activeRecon.state === 'ACTIVE' && activeRecon.duration > `${CANCEL_RECON_AFTER}`) {
                        ...
                        let seconds = Math.floor(activeRecon.duration / 1000);
                        let minutes = Math.floor(seconds / 60);
                        let hours = Math.floor(minutes / 60);
                        log.info(`Reconciliation ${activeRecon._id} has been running for : ${hours} hours - Cancelling...`)
                        ...
                        return await synthetics.executeHttpStep(`Cancel overrunning recon for mapping ${activeRecon.mapping}`, cancelRecon, validateCancelRecon, configReconDuration);
                    }
                }))
