        ...
        return new Promise((resolve, reject) => {
            ...
            res.on('end', async () => {
                ...
                JSON.parse(responseBody).reconciliations.forEach(recon => {
                    log.info(`Recon found: _id=${recon._id} - maping: ${recon.mapping} - state: ${recon.state}  - duration: ${Math.floor(recon.duration / 1000)}`)
                })
                ...
                await Promise.all(activeReconsData.map(async (activeRecon) => {
                    ...
                    if (activeRecon && activeRecon.state === 'ACTIVE' && activeRecon.duration > `${CANCEL_RECON_AFTER}`) {
                        ...
                        log.info(`Reconciliation ${activeRecon._id} has been running for : ${hours}`)
