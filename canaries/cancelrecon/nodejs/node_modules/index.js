        ...
        return new Promise((resolve, reject) => {
            ...
            res.on('end', async () => {
                ...
                await Promise.all(mappingsWithRecons.map(async (mappingWithRecon) => {
                    console.log(`Recon Cancelled: ${mappingWithRecon.recon.mapping}`)
                    if (mappingWithRecon && mappingWithRecon.recon.state === 'ACTIVE' && mappingWithRecon.recon.duration > `${CANCEL_RECON_AFTER}`) {
                        log.info(`Recon Cancelled: ${mappingWithRecon.recon.mapping}`)
                        ...
                        return await synthetics.executeHttpStep(`Verify Mapping ${mappingWithReconrecon.recon.name}`, requestReconDuration, validateReconDuration, configReconDuration);
                    }
                }))
