...
const cancelRecon = async function () {
    ...
    const validatecancelRecon = async function (res) {
        return new Promise((resolve, reject) => {
            if (res.statusCode < 200 || res.statusCode > 299) {
                throw res.statusCode + ' ' + res.statusMessage;
            }
            ...
            res.on('end', () => {
                ...
                if (mapping && mapping.recon.state === 'ACTIVE' && mapping.recon.duration > `${CANCEL_RECON_AFTER}`) {
                    ...
                    requestcancelRecon['headers']['User-Agent'] = [synthetics.getCanaryUserAgentString(), requestcancelRecon['headers']['User-Agent']].join(' ');

                    // Set step config option for Verify Mappings Recon
                    let configcancelRecon = {
                    ...
                    log.info("full request is: " + JSON.stringify(requestcancelRecon));
                    return await synthetics.executeHttpStep(`Verify Mapping ${mapping}`, requestcancelRecon, validatecancelRecon, configcancelRecon);
