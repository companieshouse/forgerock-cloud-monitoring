        ...
        return new Promise((resolve, reject) => {
            ...
            res.on('end', () => {
                ...
                if (mapping && mapping.recon && mapping.recon.state === 'SUCCESS' && percentFailed < 30) {
                    log.info(`${mapping.name} - state: ${mapping.recon.state}, percent failed: ${percentFailed}, id: ${mapping.recon._id}`)
                    resolve();
                } else {
                    log.info(`${mapping.name} - state: ${mapping.recon.state}, failures: ${mapping.recon.statusSummary.FAILURE}, percent failed: ${percentFailed}, id: ${mapping.recon._id}`)
                    reject()
                }
            });
        });
