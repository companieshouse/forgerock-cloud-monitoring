...
const rcsMonitoring = async function () {
    ...
    const validateMappingsHealth = async function (res) {
        return new Promise((resolve, reject) => {
            ...
            res.on('end', () => {
                ...
                const percentage = percentage(mapping.recon.statusSummary.FAILURE, mapping.recon.statusSummary.SUCCESS)
                if (mapping && mapping.recon.state === 'SUCCESS' && percentage < 40) {
                    resolve();
                } else {
                    reject(`${mapping.name} - state: ${mapping.recon.state}, failures: ${mapping.recon.statusSummary.FAILURE}`)
                }
            });
        });
    };
    ...
    await Promise.all(mappings.map(async (mapping) => {
      ...
      return await synthetics.executeHttpStep(`Verify Mapping ${mapping}`, requestMappingsHealth, validateMappingsHealth, configMappingsHealth);
    }))
