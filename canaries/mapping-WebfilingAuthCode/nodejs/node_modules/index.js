...
const rcsMonitoring = async function () {
    ...
    const validateMappingsHealth = async function (res) {
        return new Promise((resolve, reject) => {
            if (res.statusCode < 200 || res.statusCode > 299) {
                log.error(`CANNOT_REACH_IDM_ENDPOINT for mapping Webfiling Auth Codes ${res.statusCode} ${res.statusMessage}`)
                throw res.statusCode + ' ' + res.statusMessage;
            }
            ...
            res.on('end', () => {
                ...
                if (mapping && mapping.recon && mapping.recon.state === 'FAILED'){
                    log.info(`RECON_FAILED - ${mapping.name} - state: ${mapping.recon.state}, percent failed: ${percentFailed}, _id: ${mapping.recon._id}`)
                }
