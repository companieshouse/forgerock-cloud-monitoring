...
const rcsMonitoring = async function () {
    ...
    let accessToken;
    const { FIDC_URL, USER, PASSWORD, ADMIN_CLIENT, ADMIN_CLIENT_SECRET, CONNECTOR_GROUP } = process.env
    ...
    const validateConnectorHealth = async function (res) {
        return new Promise((resolve, reject) => {
            ...
            res.on('end', () => {
                ...
                const connector = JSON.parse(responseBody).openicf.filter(conn => conn.name === CONNECTOR_GROUP)[0]
    ...
    console.log(requestConnectorHealth)
