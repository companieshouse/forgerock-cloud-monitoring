...
const rcsMonitoring = async function () {
    ...
    const validateMapping = async function (res) {
        return new Promise((resolve, reject) => {
            if (res.statusCode < 200 || res.statusCode > 299) {
                throw res.statusCode + ' ' + res.statusMessage;
            }
    ...
    await Promise.all(mappings.map(async (mapping) => {
      ...
      return await synthetics.executeHttpStep(`Verify Mapping ${mapping}`, requestMappingsHealth, validateConnectorHealth, configConnectorHealth);
    }))
